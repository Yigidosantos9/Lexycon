...
class AddPhrasesPage extends StatefulWidget {
  @override
  State<AddPhrasesPage> createState() => _AddPhrasesPageState();
}

class _AddPhrasesPageState extends State<AddPhrasesPage> {
  ...
  void _savePhrase() {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      final category = selectedCategories.isNotEmpty
          ? selectedCategories
          : ['Uncategorized'];
      final mastery = selectedMastery.toString();
      final sentence = sentenceController.text.trim();
      final movie = movieController.text.trim();
      // Save to phrases
      StatsService.addPhrase(
        phrase,
        meaning,
        category: category,
        mastery: mastery,
      );
      // Save extra fields (sentence, movie, image) using the same key as PhraseInfoPage
      final prefsKey = 'phraseinfo_${phrase}_$meaning';
